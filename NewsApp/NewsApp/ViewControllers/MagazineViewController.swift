...
class MagazineViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        provider.request(.getMagzine, completion: getNewsType(result:))
        let label = UILabel()
    ...
    func decodeAndShow(_ response: (Response)){
        do {
            let decodeAndData = try response.map(News.self)
            self.newsListViewModel = NewsListViewModel(newsList: decodeAndData)
